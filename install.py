...
def install_react_sdk():
    ...
    install_sdk = "pushd qn-matrix-react-sdk && yarn link && yarn link matrix-js-sdk && yarn install && yarn reskindex && popd"
    install_component = subprocess.Popen(install_sdk, stdout=subprocess.PIPE, shell=True, executable="/bin/bash")
    out = install_component.stdout.read().decode()
    print(out)
...
def build_messenger():
    ...
    build_msg = "cd qn-messenger-web && yarn link matrix-js-sdk && yarn link matrix-react-sdk && yarn install && yarn reskindex && yarn dist"
